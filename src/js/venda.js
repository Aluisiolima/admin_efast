...
async function getVendasDay() {
  const vendas = await fetchApi([], "POST", `${link_api}/pegarVendas/hoje`);
  ...
  for (const chave in vendas.data) {
    ...
    vendas.data[chave].color = color;
    vendas.data[chave].id = chave;
    await render("components/cards-venda.html", vendas.data[chave], "cards");
  }
}

async function getSale(id) {
  const sales = await fetchApi([], "POST", `${link_api}/pegarVendas`);
...
async function updateStatus(id) {
  const updateStatus = await fetchApi(
    { status: "Entregue" },
    "POST",
    `${link_api}/status/${id}`
  );
